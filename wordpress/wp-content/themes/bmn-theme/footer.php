<?php
/**
 * Footer Template
 *
 * 4-column footer: Contact, Quick Links, Cities, Newsletter.
 * Copyright bar with dynamic year.
 *
 * @package bmn_theme
 * @version 2.0.0
 */

if (!defined('ABSPATH')) {
    exit;
}

$phone_number = get_theme_mod('bne_phone_number', '(617) 955-2224');
$agent_name = get_theme_mod('bne_agent_name', 'Steven Novak');
$agent_email = get_theme_mod('bne_agent_email', 'mail@steve-novak.com');
$brokerage_name = get_theme_mod('bne_brokerage_name', 'Douglas Elliman Real Estate');
$brokerage_logo = get_theme_mod('bne_brokerage_logo', '');

$instagram = get_theme_mod('bne_social_instagram', '');
$facebook = get_theme_mod('bne_social_facebook', '');
$youtube = get_theme_mod('bne_social_youtube', '');
$linkedin = get_theme_mod('bne_social_linkedin', '');

$cities = bmn_get_cities();
?>

    <!-- Footer -->
    <footer class="bg-gray-900 text-gray-300 mt-auto">
        <div class="container mx-auto px-4 lg:px-8 py-12 lg:py-16">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">

                <!-- Column 1: About / Contact -->
                <div>
                    <h3 class="text-white font-semibold text-lg mb-4">Contact Us</h3>
                    <p class="font-medium text-white"><?php echo esc_html($agent_name); ?></p>
                    <?php if ($brokerage_logo) : ?>
                        <img src="<?php echo esc_url($brokerage_logo); ?>"
                             alt="<?php echo esc_attr($brokerage_name); ?>"
                             class="h-8 w-auto mt-2 brightness-0 invert opacity-70">
                    <?php endif; ?>
                    <p class="text-sm mt-1 text-gray-400"><?php echo esc_html($brokerage_name); ?></p>

                    <?php if ($phone_number) : ?>
                        <a href="tel:<?php echo esc_attr(preg_replace('/[^0-9]/', '', $phone_number)); ?>"
                           class="block mt-3 text-sm hover:text-white transition-colors">
                            <?php echo esc_html($phone_number); ?>
                        </a>
                    <?php endif; ?>

                    <?php if ($agent_email) : ?>
                        <a href="mailto:<?php echo esc_attr($agent_email); ?>"
                           class="block mt-1 text-sm hover:text-white transition-colors">
                            <?php echo esc_html($agent_email); ?>
                        </a>
                    <?php endif; ?>

                    <!-- Social Links -->
                    <div class="flex gap-3 mt-4">
                        <?php if ($instagram) : ?>
                            <a href="<?php echo esc_url($instagram); ?>" target="_blank" rel="noopener noreferrer"
                               class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-800 text-gray-400 hover:bg-navy-700 hover:text-white transition-colors"
                               aria-label="Instagram">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                            </a>
                        <?php endif; ?>
                        <?php if ($facebook) : ?>
                            <a href="<?php echo esc_url($facebook); ?>" target="_blank" rel="noopener noreferrer"
                               class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-800 text-gray-400 hover:bg-navy-700 hover:text-white transition-colors"
                               aria-label="Facebook">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                            </a>
                        <?php endif; ?>
                        <?php if ($youtube) : ?>
                            <a href="<?php echo esc_url($youtube); ?>" target="_blank" rel="noopener noreferrer"
                               class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-800 text-gray-400 hover:bg-navy-700 hover:text-white transition-colors"
                               aria-label="YouTube">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                            </a>
                        <?php endif; ?>
                        <?php if ($linkedin) : ?>
                            <a href="<?php echo esc_url($linkedin); ?>" target="_blank" rel="noopener noreferrer"
                               class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-800 text-gray-400 hover:bg-navy-700 hover:text-white transition-colors"
                               aria-label="LinkedIn">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                            </a>
                        <?php endif; ?>
                    </div>
                </div>

                <!-- Column 2: Quick Links -->
                <div>
                    <h3 class="text-white font-semibold text-lg mb-4">Quick Links</h3>
                    <?php
                    wp_nav_menu(array(
                        'theme_location' => 'footer',
                        'menu_class'     => 'space-y-2',
                        'container'      => false,
                        'fallback_cb'    => 'bmn_footer_nav_fallback',
                        'depth'          => 1,
                        'link_before'    => '<span class="text-sm text-gray-400 hover:text-white transition-colors">',
                        'link_after'     => '</span>',
                    ));
                    ?>
                </div>

                <!-- Column 3: Search by City -->
                <div>
                    <h3 class="text-white font-semibold text-lg mb-4">Search by City</h3>
                    <?php
                    $footer_cities = !empty($cities) ? $cities : array(
                        array('name' => 'Boston'),
                        array('name' => 'Cambridge'),
                        array('name' => 'Somerville'),
                        array('name' => 'Brookline'),
                        array('name' => 'Newton'),
                        array('name' => 'Arlington'),
                        array('name' => 'Watertown'),
                        array('name' => 'Waltham'),
                    );
                    ?>
                    <ul class="space-y-2">
                        <?php foreach ($footer_cities as $city) :
                            $city_name = is_array($city) ? ($city['name'] ?? '') : $city;
                            $city_url = is_array($city) ? ($city['url'] ?? bmn_get_search_url(array('city' => $city_name))) : bmn_get_search_url(array('city' => $city));
                        ?>
                            <li>
                                <a href="<?php echo esc_url($city_url); ?>"
                                   class="text-sm text-gray-400 hover:text-white transition-colors">
                                    <?php echo esc_html($city_name); ?>
                                </a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>

                <!-- Column 4: Newsletter -->
                <div>
                    <h3 class="text-white font-semibold text-lg mb-4">Stay Updated</h3>
                    <p class="text-sm text-gray-400 mb-4">Get market updates and new listing alerts delivered to your inbox.</p>
                    <form class="space-y-3" x-data="{ email: '' }"
                          hx-post="<?php echo esc_url(rest_url('bmn/v1/newsletter/subscribe')); ?>"
                          hx-trigger="submit"
                          hx-swap="outerHTML"
                          hx-headers='{"Content-Type": "application/json"}'>
                        <input type="email"
                               name="email"
                               x-model="email"
                               placeholder="Your email address"
                               required
                               class="w-full px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white placeholder-gray-500 focus:border-navy-500 focus:ring-1 focus:ring-navy-500">
                        <button type="submit" class="w-full btn-primary text-sm !py-2.5">
                            Subscribe
                        </button>
                    </form>
                </div>
            </div>
        </div>

        <!-- Copyright Bar -->
        <div class="border-t border-gray-800">
            <div class="container mx-auto px-4 lg:px-8 py-4">
                <p class="text-center text-xs text-gray-500">
                    &copy; <?php echo date('Y'); ?> <?php echo esc_html($agent_name); ?>. All rights reserved.
                    <?php echo esc_html($brokerage_name); ?>.
                </p>
            </div>
        </div>
    </footer>

</div><!-- #page -->

<?php wp_footer(); ?>
</body>
</html>
