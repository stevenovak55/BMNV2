...
/**
 * PHPUnit Bootstrap for BMN Analytics
 *
 * Loads the platform bootstrap (WP stubs + platform autoloader),
 * then loads the analytics plugin autoloader and adds missing stubs.
 */
...
// Analytics constants.
if (! defined('BMN_ANALYTICS_VERSION')) {
    define('BMN_ANALYTICS_VERSION', '2.0.0-dev');
}

if (! defined('BMN_ANALYTICS_PATH')) {
    define('BMN_ANALYTICS_PATH', dirname(__DIR__) . '/');
}

if (! defined('BMN_ANALYTICS_URL')) {
    define('BMN_ANALYTICS_URL', 'https://bmnboston.com/wp-content/plugins/bmn-analytics/');
}

// Analytics autoloader.
$analyticsAutoloader = dirname(__DIR__) . '/vendor/autoload.php';

if (file_exists($analyticsAutoloader)) {
    require_once $analyticsAutoloader;
}
...
if (! function_exists('plugin_dir_url')) {
    ...
    {
        return 'https://bmnboston.com/wp-content/plugins/bmn-analytics/';
    }
}
